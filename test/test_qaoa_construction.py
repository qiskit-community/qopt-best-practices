...
class TestQAOAConstruction(TestCase):
    ...
    def test_depth_one(self):
        ...
        def pm_callback(**kwargs):
            print("Callback from test_depth_one:", kwargs["property_set"])

        ansatz = qaoa_pm.run(cost_op_circ, callback=pm_callback)

        print(qaoa_pm.property_set)
        #print(qaoa_pm.passes)
    ...
    def test_depth_two_qaoa_pass(self):
        ...
        value = estimator.run([(ansatz, self.cost_op, [1, 2, 3, 4])]).result()[0].data.evs
        
        library_ansatz = QAOAAnsatz(self.cost_op, reps=2)
        library_ansatz = transpile(library_ansatz, basis_gates = ["cx", "rz", "rx", "h"])

        expected = estimator.run([(library_ansatz, self.cost_op, [1, 2, 3, 4])]).result()[0].data.evs
